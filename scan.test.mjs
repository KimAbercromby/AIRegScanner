...
import { parseAtom, parseAtomChanges, parseGovukSearch, buildGovukUrl, parsePageWatch } from './fetchers.mjs';
import {
  domainAllowed,
  classify,
  passesRelevanceGate,
...
test('L2: the live log is capped by archiving old terminal records, never unreviewed ones', () => {
  const mk = (id, status) => ({ record_id: `REG-${id}`, status });
  // 5 records, keep 3: 2 must move, but only terminal ones, oldest first.
  const recs = [
    mk(1, 'reviewed'),    // oldest terminal -> archive
    mk(2, 'unreviewed'),  // never archived
    mk(3, 'retired'),     // terminal -> archive
    mk(4, 'unreviewed'),
    mk(5, 'reviewed')
  ];
  const { kept, archived } = partitionForArchive(recs, 3);
  assert.deepEqual(archived.map(r => r.record_id), ['REG-1', 'REG-3'], 'oldest terminal records are archived');
  assert.ok(kept.every(r => r.status !== 'unreviewed' ? true : true));
  assert.ok(!archived.some(r => r.status === 'unreviewed'), 'no unreviewed record is ever archived');
  assert.equal(kept.length + archived.length, recs.length, 'nothing is lost; every record is kept or archived');
});

test('L2: a log under the cap is left completely untouched', () => {
  const recs = [{ record_id: 'REG-1', status: 'reviewed' }, { record_id: 'REG-2', status: 'unreviewed' }];
  const { kept, archived } = partitionForArchive(recs, 3000);
  assert.equal(archived.length, 0);
  assert.equal(kept, recs, 'the same array is returned untouched');
});

test('H1: the deep body-hash control is switched on for at least one source', () => {
  const reg = JSON.parse(readFileSync(join(HERE, 'sources.json'), 'utf8'));
  const deep = reg.sources.filter((s) => s.deep === true);
  assert.ok(deep.length >= 1, 'at least one source must be marked deep so the rewrite control is not dormant');
  const runnable = new Set(['verified', 'form-verified']);
  assert.ok(deep.every((s) => runnable.has(s.verification_status)),
    'a deep source must be runnable, or the control never fires');
});

test('page-watch returns one stable item whose descriptor carries the source keywords', () => {
  const source = {
    url: 'https://www.iso.org/standard/42001',
    watch_title: 'ISO/IEC 42001 (AI management system standard)',
    watch_summary: 'ISO/IEC 42001 artificial intelligence management system standard catalogue page.'
  };
  const items = parsePageWatch('<html><head><title>Whatever the page says today</title></head><body>x</body></html>', source);
  assert.equal(items.length, 1, 'the page is a single watched item');
  assert.equal(items[0].raw_id, source.url, 'identity is the page URL, so it is stable across runs');
  assert.equal(items[0].title, source.watch_title, 'the configured title is used, not the volatile page title');
  assert.match(items[0].summary.toLowerCase(), /iso\/iec 42001/, 'the descriptor carries the keyword the relevance gate needs');
});

test('page-watch standards are watched by the focus gate and close the framework-map gaps', () => {
  const reg = JSON.parse(readFileSync(join(HERE, 'sources.json'), 'utf8'));
  const mappings = JSON.parse(readFileSync(join(HERE, 'mappings.json'), 'utf8'));
  for (const id of ['iso-42001', 'nist-ai-rmf']) {
    const src = reg.sources.find((s) => s.id === id);
    assert.ok(src, `${id} exists`);
    assert.equal(src.type, 'page-watch');
    assert.ok(src.deep === true, `${id} must be deep so content revisions are caught`);
    const item = { title: src.watch_title, summary: src.watch_summary, url: src.url };
    const decision = sourceRelevanceDecision(src, item, mappings);
    assert.ok(decision.keep, `${id} must be kept by the gate`);
    assert.ok(decision.priority.length > 0, `${id} must land as a priority framework`);
  }
});
